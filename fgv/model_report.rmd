...
```{r}
data <- read_csv("data/treated/train/409_train_data_treated.csv")

# Print the first 6 rows of the data
head(data)

# Set the default theme to theme_minimal
...
plot_time_distance <- data %>% ggplot(aes(x = time_traveled, y = distance_traveled, color = as.factor(direction))) +
  geom_point() +
...
plot_filtered_data <- filtered_data %>% ggplot(aes(x = time_traveled_trip, y = distance_traveled, color = as.factor(direction))) +
...
p1 <- ggplot(filtered_data, aes(x = mean_speed_1_min)) +
  geom_histogram(bins = 30, fill = "blue", color = "black") +
  labs(title = "Velocidade média com 1 minuto de intervalo", x = "Velocidade média (km/h)", y = "Frequência") +
  ylim(0, max_bin_count*1.1) +
    theme_minimal()

p2 <- ggplot(filtered_data, aes(x = mean_speed_3_min)) +
    geom_histogram(bins = 30, fill = "blue", color = "black") +
    labs(title = "Velocidade média com 3 minutos de intervalo", x = "Velocidade média (km/h)", y = "Frequência") +
    ylim(0, max_bin_count*1.1) +
    theme_minimal()

p3 <- ggplot(filtered_data, aes(x = mean_speed_5_min)) +
    geom_histogram(bins = 30, fill = "blue", color = "black") +
    labs(title = "Velocidade média com 5 minutos de intervalo", x = "Velocidade média (km/h)", y = "Frequência") +
    ylim(0, max_bin_count*1.1) +
    theme_minimal()

p4 <- ggplot(filtered_data, aes(x = velocidade_estimada_10_min)) +
    geom_histogram(bins = 30, fill = "blue", color = "black") +
    labs(title = "Velocidade média com 10 minutos de intervalo", x = "Velocidade média (km/h)", y = "Frequência") +
    ylim(0, max_bin_count*1.1) +
    theme_minimal()
...
linear_model <- lm(time_traveled_trip ~ distance_traveled, data = filtered_data)

# Print the summary of the linear model
summary(linear_model)

filtered_data$predicted_time_traveled <- predict(linear_model, newdata = filtered_data)
```

Cria um modelo para cada velocidade média estimada (1, 3, 5 e 10 minutos) sendo a distância percorrida uma variável independente.

```{r}
# Fit a linear model to predict the time traveled based on the distance traveled and the mean speed at 1 minute
linear_model_speed_1_min <- lm(time_traveled_trip ~ distance_traveled + distance_traveled:mean_speed_1_min, data = filtered_data)

# Fit a linear model to predict the time traveled based on the distance traveled and the mean speed at 3 minutes
linear_model_speed_3_min <- lm(time_traveled_trip ~ distance_traveled + distance_traveled:mean_speed_3_min, data = filtered_data)

# Fit a linear model to predict the time traveled based on the distance traveled and the mean speed at 5 minutes
linear_model_speed_5_min <- lm(time_traveled_trip ~ distance_traveled + distance_traveled:mean_speed_5_min, data = filtered_data)

# Fit a linear model to predict the time traveled based on the distance traveled and the mean speed at 10 minutes
linear_model_speed_10_min <- lm(time_traveled_trip ~ distance_traveled + distance_traveled:velocidade_estimada_10_min, data = filtered_data)

# Print all the summaries of the linear models with the mean speed at 1, 3, and 5 minutes
summary(linear_model_speed_1_min)
summary(linear_model_speed_3_min)
summary(linear_model_speed_5_min)
summary(linear_model_speed_10_min)

# Create a table with the estimated coefficients of the linear models and their r-squared values
coefficients_table <- data.frame(
  model = c("linear_model", "linear_model_speed_1_min", "linear_model_speed_3_min", "linear_model_speed_5_min", "linear_model_speed_10_min"),
  r_squared = c(summary(linear_model)$r.squared, summary(linear_model_speed_1_min)$r.squared, summary(linear_model_speed_3_min)$r.squared, summary(linear_model_speed_5_min)$r.squared, summary(linear_model_speed_10_min)$r.squared),
  intercept = c(coef(linear_model)["(Intercept)"], coef(linear_model_speed_1_min)["(Intercept)"], coef(linear_model_speed_3_min)["(Intercept)"], coef(linear_model_speed_5_min)["(Intercept)"], coef(linear_model_speed_10_min)["(Intercept)"]),
  distance_traveled = c(coef(linear_model)["distance_traveled"], coef(linear_model_speed_1_min)["distance_traveled"], coef(linear_model_speed_3_min)["distance_traveled"], coef(linear_model_speed_5_min)["distance_traveled"], coef(linear_model_speed_10_min)["distance_traveled"]),
  average_speed = c(NA, coef(linear_model_speed_1_min)["mean_speed_1_min"], coef(linear_model_speed_3_min)["mean_speed_3_min"], coef(linear_model_speed_5_min)["mean_speed_5_min"], coef(linear_model_speed_10_min)["velocidade_estimada_10_min"])
)
...
interaction_model <- glm(time_traveled_trip ~ distance_traveled:as.factor(direction), data = filtered_data)
...
speed_model <- lm(time_traveled_trip ~ distance_traveled:as.factor(direction) + distance_traveled:as.factor(direction):mean_speed_5_min, data = filtered_data)
...
plot_predicted_time <- filtered_data %>% ggplot(aes(x = time_traveled_trip, y = predicted_time_traveled)) +
  ...
  labs(title = "Tempo de viagem real vs Tempo de viagem previsto", x = "Tempo de viagem real (s)", y = "Tempo de viagem previsto (s)")+
    theme_minimal()
...
plot_errors <- filtered_data %>% filter(distance_traveled <= 10000) %>% ggplot(aes(x = cut(distance_traveled, breaks = seq(0, 10000, by = 500)), y = error)) +
  ...
  labs(title = "Erros do modelo para cada intervalo de 500 metros percorridos", x = "Distância percorrida (m)", y = "Erro (s)") + ylim(-1200, 1200) +
    theme_minimal()
...
gamma_model <- glm(time_traveled_trip ~ distance_traveled:as.factor(direction) + distance_traveled:as.factor(direction):mean_speed_5_min, data = filtered_data, family = "Gamma")
...
plot_errors_gamma <- filtered_data %>% filter(distance_traveled <= 10000) %>% ggplot(aes(x = cut(distance_traveled, breaks = seq(0, 10000, by = 500)), y = error_gamma)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(title = "Erros do modelo gamma para cada intervalo de 500 metros percorridos", x = "Distância percorrida (m)", y = "Erro (s)") +
   ylim(-1200, 1200) +
    theme_minimal()
...
plot_errors_week_day <- filtered_data %>% ggplot(aes(x = week_day, y = error)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(title = "Erros do modelo para cada dia da semana", x = "Dia da semana", y = "Erro (s)") + ylim(-2400, 2400) +
    theme_minimal()

plot_errors_time_period <- filtered_data %>% ggplot(aes(x = time_period, y = error)) +
    geom_boxplot(fill = "blue", color = "black") +
    labs(title = "Erros do modelo para cada período do dia", x = "Período do dia", y = "Erro (s)") + ylim(-2400, 2400) +
    theme_minimal()
...
weekend_model <- lm(time_traveled_trip ~ distance_traveled:as.factor(direction):as.factor(is_weekend) + distance_traveled:as.factor(direction):mean_speed_5_min, data = filtered_data)
...
plot_errors_week_day_distance <- filtered_data %>% filter(distance_traveled <= 10000) %>% ggplot(aes(x = cut(distance_traveled, breaks = seq(0, 10000, by = 500)), y = error_weekend)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(title = "Erros do modelo para cada dia da semana e cada intervalo de 500 metros percorridos", x = "Distância percorrida (m)", y = "Erro (s)") + ylim(-1200, 1200) + facet_wrap(~week_day) +
    theme_minimal()
...
plot_errors_time_period_week_day <- filtered_data %>% ggplot(aes(x = week_day, y = error_weekend)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(title = "Erros do modelo para cada dia da semana e cada período do dia", x = "Dia da semana", y = "Erro (s)") +
  ylim(-1200, 1200) + facet_wrap(~time_period) +
    theme_minimal()
...
time_period_model <- lm(time_traveled_trip ~ distance_traveled:as.factor(direction):as.factor(is_weekend) + distance_traveled:as.factor(direction):mean_speed_5_min + time_period:as.factor(direction) -1, data = filtered_data)
