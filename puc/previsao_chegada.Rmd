...
- Algumas observações tinham tempos de chegada de muitas horas

- Criamos alguns filtros para:

    - Ônibus que desaparecem por alguns minutos
    
    - Ônibus que não mudam de posição
    
    - Ônibus distantes do itinerário
    
- Nessas observações (túneis, garagem, linha incorreta), não conseguimos identificar os tempos de chegada    
...
- Perda de cerca de 25%

- Natural que nem todas sejam identificáveis, mas deve haver problemas com alguns serviços
...
- Estima muitas árvores de regressão com subamostras; tira a média

- Captura relações não-lineares e padrões complexos

```{r, out.width="80%", fig.cap="Árvore de regressão para o 309"}
knitr::include_graphics("output/decision_tree.png")
```

## Índice de importância das variáveis

```{r, out.width="80%", fig.cap="Linha 309"}
knitr::include_graphics("output/plot_variable_importance.png")
```

## Random Forest: erros de previsão
