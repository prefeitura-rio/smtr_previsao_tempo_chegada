...
```{r, message = FALSE}
training_data <- fread(file.path(source, "gps_training_proj.csv"))

test_data <- fread(file.path(source, "gps_test_proj.csv"))
```
...
bench <- training_data %>%
    summarise(
        ...
        .by = c("servico", "next_stop_id")
    )

bench <- test_data %>%
    left_join(bench, by = c("servico", "next_stop_id"))
...
fe_reg <- feols(
    log(arrival_time) ~ log(dist_next_stop) + log(velocidade_estimada_10_min) 
    + log(velocidade_instantanea + 1) | servico^next_stop_id,
    data = training_data,
    combine.quick = FALSE
)
...
fe_reg_hora <- feols(
    log(arrival_time) ~ log(dist_next_stop) + log(velocidade_estimada_10_min)
    + log(velocidade_instantanea + 1) | servico^next_stop_id + next_stop_id^hora,
    data = training_data,
    combine.quick = FALSE
)
